...
const ImageGallery = ({ images }) => {
  ...
  return (
    <div className="image-gallery block lg:grid lg:grid-cols-12 lg:grid-rows-4 gap-5">
      <div
        className={`current-image h-56 mb-2 md:h-72  ${
      ...
      <div
        className="gallery block overflow-x-scroll lg:overflow-hidden relative lg:pt-12 col-span-12  bg-purple-50  lg:col-span-2 lg:row-span-4  p-5 shadow rounded "
        ref={containerRef}
      >
        {!isSmallDevice && (
          <>
        ...
        >
          {images.map((image) => {
            return (
              <div
                key={image}
                className={`gallery-item  flex-shrink-0 ml-5 lg:ml-0  h-20 relative w-40  lg:w-auto  rounded-lg lg:h-36  mb-3 cursor-pointer ${
                  image === currentImage
